...
TINY.table=function(){
	...
	//	this.l=num;
		...
		t.w=t.h.cells.length;						//document.getElementById("kw").value=t.w;
		for(i;i<t.w;i++){
			var c=t.h.cells[i];
	...
	sorter.prototype.wk=function(y){
		...
		//change background of  the table title 
		for(i=0;i<t.w;i++){
			var c=t.h.cells[i];
			if(c.className!='nosort'){
				c.className=this.head;
		...
		if(t.p==y){
			t.a.reverse();
			x.className=t.d?this.asc:this.desc;t.d=t.d?0:1;
		}
		else{
			t.p=y;t.a.sort(cp);
			t.d=0;x.className=this.asc;
		}
		...
		sorter.prototype.size=function(s){
			this.pagesize=s;this.g=1;this.pages();
			this.page(0);if(this.currentid&&this.limitid){
