...
function setTable(header,documentNo){
	...
	_row=0;
	searchRemark(documentNo);
}
function searchRemark(documentNo){
	$.ajax({
		type:"post",
		url:"./SearchRemarks",
		...
		{
			document.getElementById("remarks").value=data;
		}
	});
}
...
function saveChange(){
	...
	$.ajax({
		type:"post",
		url:"./SaveRemarks",
		data:{"documentNo":$("#documentNo").val(),"Header":fill_document(_row,"documentHeader"),"remarks":remarks},
		success:function(data) //把数据保存为json
		{
			if(data==1){
				alert("保存成功");
